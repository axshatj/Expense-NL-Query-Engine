...
- **`subscriptions`** — anything recurring (same normalized merchant, same-ish amount, monthly cadence). Worth detecting explicitly with a lightweight recurrence check — same merchant + amount within a small tolerance appearing in ≥2 consecutive months — since "what subscriptions am I paying for" is one of the highest-value questions this whole project can answer, and a plain category tag alone doesn't get you there; it needs the recurrence signal too. **This recurrence check is used twice, not once**: at ingestion time to pre-tag likely subscriptions, and again at query time (see `nl-query-and-grounding.md`'s subscription query builder) to catch recurring charges that were mistagged under a different category at ingestion — relying on the ingestion-time tag alone was the cause of a real bug where "how much are all my subscription costs?" silently undercounted by missing mistagged recurring transactions.
...
## When a question's terms don't match the taxonomy

Users won't phrase things in taxonomy language — "perfumes," "gym," "cab" aren't valid categories, but they're legitimate things to ask about. The query layer (see `nl-query-and-grounding.md`) handles this with two explicit outcomes rather than one silent guess: map to the closest category and *say so* in the answer, or admit no confident match exists and ask the user to clarify rather than running a query against a forced, arbitrary category. Never let a mismatch between the user's words and the fixed taxonomy silently turn into a confidently-stated number for a different question than the one asked — that's the difference between a useful mapping and a wrong answer that merely looks precise.
